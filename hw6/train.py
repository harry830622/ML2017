...
if __name__ == "__main__":
    ...
    # splited_training_x = splited_training_x.reshape(
    #     splited_training_x.shape[0], 2, 1)
    splited_training_y = training_y[:-num_validating_x]
    splited_validating_x = training_x[-num_validating_x:]
    # splited_validating_x = splited_validating_x.reshape(
    #     splited_validating_x.shape[0], 2, 1)
    ...
    history = model.fit(
        ...
        epochs=100,
        callbacks=[
            EarlyStopping(monitor="val_mean_squared_error", patience=5, verbose=1),
            ModelCheckpoint(
                "model.h5",
                save_best_only=True,
