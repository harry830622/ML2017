...
sigma1 /= num_true
sigma2 /= num_false
shared_sigma = num_true / float(
    num_training_data) * sigma1 + num_false / float(num_training_data) * sigma2
shared_sigma_inverse = shared_sigma.getI()
weights = (u1 - u2).dot(shared_sigma_inverse).transpose()
bias = (-0.5) * u1.dot(shared_sigma_inverse).dot(u1.transpose(
)) + 0.5 * u2.dot(shared_sigma_inverse).dot(u2.transpose()) + np.log(
    num_true / float(num_false))
