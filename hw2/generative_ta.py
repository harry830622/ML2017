...
def load_data(x_train_file_name, y_train_file_name, x_test_file_name):
    X_train = pd.read_csv(x_train_file_name)
    X_test = pd.read_csv(x_test_file_name)
    Y_train = np.zeros((len(X_train), ))
    with open(y_train_file_name, 'r') as fp:
...
def predict(X_test, mu1, mu2, shared_sigma, N1, N2):
    ...
    x = X_test.T
    b = (-0.5) * np.dot(np.dot([mu1], sigma_inverse), mu1) + (0.5) * np.dot(
        np.dot([mu2], sigma_inverse), mu2) + np.log(float(N1) / N2)
    a = np.dot(w, x) + b
    y = sigmoid(a)
    return y
...
def train(X_train, Y_train):
    ...
    for i in range(train_data_size):
        if Y_train[i] == 1:
            sigma1 += np.dot(
                np.transpose([X_train[i] - mu1]), [(X_train[i] - mu1)])
        else:
            sigma2 += np.dot(
                np.transpose([X_train[i] - mu2]), [(X_train[i] - mu2)])
    sigma1 /= cnt1
    sigma2 /= cnt2
...
if __name__ == '__main__':
    ...
    (X_train, Y_train, X_test) = load_data(x_train_file_name,
                                           y_train_file_name, x_test_file_name)
    #X_train, X_test = normalize(X_train, X_test)
    mu1, mu2, shared_sigma, N1, N2 = train(X_train, Y_train)
    y = predict(X_train, mu1, mu2, shared_sigma, N1, N2)
    y_ = np.around(y)
    result = (Y_train == y_)
    print('ACCURACY: %f' % (float(result.sum()) / result.shape[0]))
    y = predict(X_test, mu1, mu2, shared_sigma, N1, N2)
    y_ = np.around(y)

    output_file_name = sys.argv[4]
    sample_submission(y_, output_file_name)
